...
def extract_kart_objects(
    ...
    for det in info["detections"][view_index]:
        ...
        if (sx2 - sx1) < min_box_size or (sy2 - sy1) < min_box_size:
            continue
        if sx2 < 0 or sx1 > img_width or sy2 < 0 or sy1 > img_height:
            continue
    ...
    if karts:
        best = min(karts, key=lambda k: (k["center"][0] - img_cx) ** 2 + (k["center"][1] - img_cy) ** 2)
    ...
    for info_file in info_files:
        ...
        for view_index in range(10):
            ...
            for qa in qa_pairs:
                ...
                all_qa.append(qa)
        if all_qa:
            output_file = Path(data_dir) / f"{base_name}_qa_pairs.json"
            with open(output_file, "w") as f:
                json.dump(all_qa, f)
    print(f"Generated QA pairs for {len(info_files)} info files")
